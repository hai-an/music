<template>
    <div class="play">
        ...
        <div
            class="song-bg"
            :style="`background-image: url(${songInfo && songInfo.al && songInfo.al.picUrl
        ...
        <div class="song-wrapper">
            ...
            <!-- 播放按钮 -->
            <div class="start-box" @click="audioStart" @load="onLoad">
...
export default {
    ...
    methods: {
        ...
        { // 获取歌曲详情, 和歌词方法
            ...
            console.log( lyrContent.data.lrc.lyric );/* 拿到最初的歌词---字符串 */
            const lyricStr = lyrContent.data.lrc.lyric/* 设置一个变量接收 */
            this.lyric = this._formatLyr( lyricStr )/* 格式化函数 */

            console.log( '打印lyric--歌词枚举对象' );
        ...
        _formatLyr ( lyricStr )
        {
            ...
            // let reg = /\[.+?\]/g  /*  .任何字符, +一个或多个,? 非贪婪模式 0-1 之间 */
            let reg = /\[\S+?\]/g/* \S匹配任何非空白字符 */
            // let reg = /\[[\s\S]+?\]/g/* 匹配[里面的所有内容,用[\s\S]+是贪婪匹配,直接变成一条长度,?非贪婪模式 0或者1,] */
            ...
            let contentArr = lyricStr.split( /\[\S+?\]/ ).slice( 1 )
            ...
            timeArr.forEach( ( item,index ) =>
            {
                ...
                console.log( ms );
                // let hh = item.split( ':' )/* 分割2个长度的数组 0: [00: ,1:00.000] */
                // let hh = item.split( ':' )[ 1 ]/*把 00.000]拿出来 */
                // let hh = item.split( ':' )[ 1 ].split( '.' )[ 0 ]/*  取00.  丢000] */
                // let hh = item.split( ':' )[ 1 ].split( '.' )[ 0 ].split('')/*  取00=>  0 0   */
                let hh = item.split( ':' )[ 1 ].split( '.' )[ 0 ].split( '' )[ 0 ] /* 00=>  取0  丢掉0 */
                ...
                // 如果十位的秒===0 ,取秒数的个位数,否则取秒的十位数和个位数: item.split( ':' )[ 1 ].split( '.' )[ 0 ]
                // 秒数作为key, 对应歌词作为value
        ...
        onLoad ()
        {
            let autoPlay = document.querySelector( '.start-box' )
